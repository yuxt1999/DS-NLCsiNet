...
from keras.layers import Input, Dense, BatchNormalization, Reshape, Conv2D, add, LeakyReLU, concatenate
...
def residual_network(x, dense_num, encoded_dim):
    ...
    def dense_residual_block(y):
        ...
        y = concatenate(layers_concat, axis=1)
        y = add_common_layers(y)
        ...
        y = concatenate(layers_concat, axis = 1)
        y = add_common_layers(y)
        y = Conv2D(2, (3, 3), padding='same', data_format="channels_first")(y)
        layers_concat.append(y)
...
network_output = residual_network(image_tensor, dense_num, encoded_dim)
...
history = LossHistory()
file = 'DS-NLCsiNet_'+(envir)+'_dim'+str(encoded_dim)+'_'+str(compression_rate)+'_'+(non_local_mode)+time.strftime('_%m_%d')
...
print("When compression is", compression_rate, ", mode is", non_local_mode)
